...
function splitStudentNameParts(studentName = '', studentSurname = '') {
  ...
  const normalizedSurname = String(studentSurname ?? '').trim();
...
function buildParentCredentials({ parentName, parentPhone, parentPassword, username }) {
  const finalUsername = buildGeneratedUsernameFromName(username, parentName) || normalizeWhatsappNumber(parentPhone) || `VELI-${Date.now()}`;
...
function ensureParentCredentials({ parentName, parentPhone, parentPassword, username }) {
  const credentials = buildParentCredentials({ parentName, parentPhone, parentPassword, username });
  const sanitizedPassword = String(credentials.password ?? '').trim();

  if (!sanitizedPassword) {
    return {
      ...credentials,
      password: '',
      username: String(credentials.username ?? '').trim() || generateUsername(parentName || '') || `VELI-${Date.now()}`,
    };
  }
...
function buildGeneratedUsernameFromName(value, fallback = '') {
  const sourceName = String(value ?? '').trim();
  if (sourceName) return generateUsername(sourceName);
  if (fallback) return generateUsername(fallback);
  return '';
}
...
function resolveBranchIdList(value) {
  ...
  if (typeof value === 'string') {
    ...
    try {
      ...
      // Ignore JSON parsing failures and fall back to comma-splitting.
    }
  ...
  if (typeof value === 'object') {
    return Object.values(value)
      .flatMap((item) => resolveBranchIdList(item))
      .filter(Boolean);
  }
...
function getStudentBranchIds(student) {
  ...
  const mergedIds = rawIds.flatMap((value) => resolveBranchIdList(value));
  const uniqueIds = [...new Set(mergedIds.map((id) => String(id).trim()))].filter(Boolean);

  return uniqueIds;
}

function studentMatchesBranch(student, branchId) {
  if (!branchId) return true;

  const selectedBranchId = String(branchId).trim();
  if (!selectedBranchId) return true;

  const branchIds = getStudentBranchIds(student).map((id) => String(id).trim());
  const directMatches = branchIds.includes(selectedBranchId) ||
    (student?.branch_id && String(student.branch_id).trim() === selectedBranchId) ||
    (student?.branchId && String(student.branchId).trim() === selectedBranchId);

  if (directMatches) return true;

  const jsonBranchIds = resolveBranchIdList(student?.branch_ids ?? student?.branchIds ?? []);
  return jsonBranchIds.some((id) => String(id).trim() === selectedBranchId);
}
...
function getCurrentParentStudentMatches(currentUser, clubsList = []) {
  ...
  const directParentPhone = normalizeWhatsappNumber(String(currentUser.phone || '').trim());

  if (!parentProfileIds.length && !childStudentIds.length && !directParentName && !directParentUsername && !directParentPhone) {
    return [];
  }
  ...
  return clubsList
    ...
    .filter((student) => {
      ...
      const studentParentPhone = normalizeWhatsappNumber(String(student?.parentPhone ?? student?.parent_phone ?? '').trim());
      ...
      const matchesDirectPhone = Boolean(directParentPhone && studentParentPhone && directParentPhone === studentParentPhone);

      return matchesParentId || matchesChildStudent || matchesDirectName || matchesDirectUsername || matchesDirectPhone;
    });
}
...
function AppClean({ initialPublicClubId = null } = {}) {
  ...
  useEffect(() => {
    ...
    const sessionUser = currentUser || (() => {
      ...
      try {
        ...
        console.warn('Parent session restore failed:', error);
        return null;
      }
    })();
    ...
    const isParentSession = ['parent', 'veli'].includes(String(sessionUser.role ?? '').trim().toLowerCase());
    if (!isParentSession) {
      setParentViewProfile(null);
      setParentViewStudents([]);
      return undefined;
  ...
  useEffect(() => {
    ...
    const loadCoachProfiles = async () => {
      ...
      try {
        ...
        if (activeClubId) {
          coachQuery = coachQuery.eq('club_id', activeClubId);
        }

        const coachRowsResult = await coachQuery.order('created_at', { ascending: false });
        ...
        const directCoachRecords = (coachRows ?? []).map((row) => {
          const clubId = normalizeDbClubId(row.club_id || resolvedSelectedClubId || currentUser?.clubId || clubs[0]?.id || '') || row.club_id || resolvedSelectedClubId || currentUser?.clubId || clubs[0]?.id || '';
          const clubBranches = clubs.find((club) => club.id === clubId)?.branches ?? [];
          const matchedBranch = clubBranches.find((branch) => branch.id === row.branch_id) ?? clubBranches[0] ?? null;

          return {
            id: row.id,
            role: 'coach',
            name: row.name || row.username || 'Antrenör',
            username: row.username || '',
            password: row.password || '',
            clubId,
            branchId: row.branch_id || matchedBranch?.id || null,
            branchName: matchedBranch?.name || 'Belirtilmemiş',
            phone: row.phone || '',
            email: '',
        ...
        const profileCoachRecords = (profileRows ?? [])
          .filter((row) => {
        ...
        const mergedCoachMap = new Map();
        [...directCoachRecords, ...profileCoachRecords].forEach((coach) => {
          const key = coach.id ? `${coach.clubId}::${coach.id}` : `${coach.clubId}::${String(coach.name || '').trim().toLowerCase()}`;
          const existing = mergedCoachMap.get(key);
          mergedCoachMap.set(key, { ...(existing ?? {}), ...coach });
        });

        const normalizedCoaches = Array.from(mergedCoachMap.values());
  ...
  const handleAddCoach = async () => {
    ...
    try {
      ...
      if (!coachInsertResult.ok) {
        throw new Error(coachInsertResult.error?.message || 'Antrenör kulüp kaydı oluşturulamadı.');
      }
      ...
      <div className="space-y-6">
            ...
            <div className="card-surface rounded-3xl p-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                >
                  ...
                  {(currentClub?.branches ?? []).map((branch, index) => (
                    <option key={`${branch.id ?? 'branch'}-${index}`} value={branch.id}>{branch.name}</option>
                  ))}
                </select>
                <input
                  className="input-shell"
              ...
              <div className="mt-5 flex justify-end">
                <button
                  ...
                  onClick={async () => {
                    ...
                    const studentNameParts = splitStudentNameParts(applicationForm.studentName, applicationForm.studentSurname);
        ...
        {managerTab === 'pending' && (
          <div className="card-surface rounded-3xl p-4 sm:p-6">
            ...
            <div className="space-y-3">
              {bekleyenler.length ? (
                bekleyenler.map((application, applicationIndex) => (
                  <div key={`${application.id ?? 'application'}-${applicationIndex}`} className="rounded-2xl border border-slate-700 bg-slate-900/70 p-4">
          ...
          <div className="card-surface rounded-3xl p-4 sm:p-6">
            ...
            <div className="space-y-3">
              <select className="input-shell" value={announcementForm.target} onChange={(e) => setAnnouncementForm({ ...announcementForm, target: e.target.value })}>
                {announcementTargets.map((option, optionIndex) => (
                  <option key={`${option.value ?? 'announcement-target'}-${optionIndex}`} value={option.value}>{option.label}</option>
                ))}
              </select>
  ...
  function CoachPanel({
    ...
    const resolvedCoachBranchId = (() => {
      const directMatch = resolvedCoachBranchCandidates.find((candidate) => {
        if (typeof candidate !== 'string') return false;
        if (validBranchIds.has(candidate)) return true;
        return coachClub?.branches?.some((branch) => String(branch.name).trim().toLowerCase() === String(candidate).trim().toLowerCase());
      });

      if (!directMatch) return '';

      if (validBranchIds.has(directMatch)) return directMatch;

      const branchByName = coachClub?.branches?.find((branch) => String(branch.name).trim().toLowerCase() === String(directMatch).trim().toLowerCase());
      return branchByName?.id || '';
    })();

    const selectedCoachBranchId = resolvedCoachBranchId || (
      selectedCoach?.branchName
    ...
    const allCoachStudents = coachClub?.students.filter((student) => {
      const isActiveStudent = !student.status || ['active', 'aktif'].includes(String(student.status).trim().toLowerCase());
      return isActiveStudent && studentMatchesBranch(student, selectedCoachBranchId);
    }) ?? [];
    const todayIso = new Date().toISOString().slice(0, 10);
    const coachStudents = allCoachStudents;
  ...
  const renderParentPanel = () => {
    ...
    const parentScopedStudents = (activeParentClub?.students ?? []).filter((student) => {
      ...
      const studentParentName = normalizeDuplicateText(String(student?.parentName ?? student?.parent_name ?? '').trim());
      const studentParentPhone = normalizeWhatsappNumber(String(student?.parentPhone ?? student?.parent_phone ?? '').trim());
      const currentParentName = normalizeDuplicateText(String(currentUser?.name || currentUser?.full_name || currentUser?.username || '').trim());
      const currentParentPhone = normalizeWhatsappNumber(String(currentUser?.phone || '').trim());
      const currentParentUsername = normalizeLoginUsername(String(currentUser?.username || '').trim());
      ...
      if (isPlainParentSession) {
        ...
        const matchedByName = Boolean(currentParentName && studentParentName && currentParentName === studentParentName);
        const matchedByPhone = Boolean(currentParentPhone && studentParentPhone && currentParentPhone === studentParentPhone);
        const matchedByUsername = Boolean(currentParentUsername && studentParentName && currentParentUsername === normalizeLoginUsername(studentParentName));

        return matchedByParentId || matchedByChildStudentId || matchedByName || matchedByPhone || matchedByUsername;
      }
          ...
          {adminFilterEnabled && (
            <div className="mt-5 grid gap-3 md:grid-cols-3">
              <label className="space-y-2">
                ...
                >
                  <option value="">Kulüp Seç</option>
                  {availableClubOptions.map((club, clubIndex) => (
                    <option key={`${club.id ?? 'club-option'}-${clubIndex}`} value={club.id}>{club.name}</option>
                  ))}
                </select>
              </label>
              ...
              <div className="space-y-3">
                {parentNotifications.length ? (
                  parentNotifications.map((notification, index) => {
                    const notificationType = String(notification?.type || '').trim().toLowerCase();
                    const rawText = String(notification?.text || '').trim();
                    const label = notificationType.includes('payment')
                      ? 'ÖDEME HATIRLATMASI'
                      : (notificationType.includes('announcement') || notificationType.includes('duyuru') || notificationType.includes('notice') || rawText.toLowerCase().includes('duyuru') || rawText.toLowerCase().includes('announcement'))
                        ? 'DUYURU'
                        : 'BİLDİRİM';

                    return (
                      <div key={`${notification.id ?? 'notification'}-${index}`} className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4">
                        <div className="mb-1 flex items-center justify-between gap-3">
                          <span className="text-xs uppercase tracking-[0.2em] text-amber-200">{label}</span>
  ...
  const persistParentToSupabase = async ({ clubId, name, phone, username, password }) => {
    ...
    if (!rawPassword) {
      return { ok: false, error: new Error('Veli şifresi boş olamaz. Lütfen kullanıcıdan güvenli bir şifre alın.') };
    }
    ...
    return (
      <div className="mx-auto max-w-5xl px-4 py-8">
        <div className="card-surface rounded-[32px] p-6 md:p-8">
          ...
          >
            <option value="" disabled>Branş Seçiniz</option>
            {formClub?.branches.map((branch, branchIndex) => (
              <option key={`${branch.id ?? 'form-branch'}-${branchIndex}`} value={branch.id}>{branch.name}</option>
            ))}
          </select>
          <input
            className="input-shell"
                    ...
                    <label className="flex items-center gap-2 rounded-2xl border border-slate-700 bg-slate-950/70 px-3 py-2">
                      ...
                      >
                        {clubs.map((club, clubIndex) => (
                          <option key={`${club.id ?? 'club'}-${clubIndex}`} value={club.id}>{club.name}</option>
                        ))}
                      </select>
                    </label>
                  )}
                ...
                <label className="flex items-center gap-2 rounded-2xl border border-slate-700 bg-slate-950/70 px-3 py-2">
                  ...
                  >
                    {clubs
                      ...
                      .map((club, clubIndex) => (
                        <option key={`${club.id ?? 'active-club'}-${clubIndex}`} value={club.id}>{club.name}</option>
                      ))}
                  </select>
                </label>
              )}
