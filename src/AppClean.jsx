...
function ensureParentCredentials({ parentName, parentPhone, parentPassword, username }) {
  ...
  if (!sanitizedPassword) {
    const fallbackPassword = createAutoGeneratedParentPassword(parentPhone, parentName);
    return {
      ...credentials,
...
function createAutoGeneratedParentPassword(parentPhone = '', parentName = '') {
  const rawSeed = normalizeWhatsappNumber(parentPhone) || generateUsername(parentName || '') || `VELI-${Date.now()}`;
  const seed = String(rawSeed).replace(/[^A-Za-z0-9]/g, '').slice(-10) || 'VELI';
  return `${seed}${generatePassword(8)}`.slice(0, 18);
}
